...
export const BEAMDOG_NWMASTER_JSON_API = 'https://api.nwn.beamdog.net/v1/servers/';
...
export async function fetchServer<M>(
  identifier: string,
  mapper?: BeamdogMapper<M>,
): Promise<BeamdogApiResponseBody | M> {
  const url = `${BEAMDOG_NWMASTER_JSON_API}${(
    (isValidBeamdogDbKey(identifier) && identifier) ||
    (isValidIPAndPort(identifier) && identifier.replace(':', '/'))
  )}`;

  const response = await fetch(url);
  if (response.status !== 200) throw new BeamdogApiError(response, await response.text());

  const json = await response.json() as BeamdogApiResponseBody;
  return mapper
